...
fetch("https://localhost:7033/api/Message/getMessages?id=" + id, {
  ...
  .then((data) => {
    ...
    data.forEach((student) => {
      ...
      fetchStudentDetails(student.id, isread);
    });
  })
  .catch((error) => {
...
function fetchStudentDetails(userId, isread) {
  fetch("https://localhost:7033/api/Manager/getStudent?id=" + userId, {
    method: "GET",
    headers: {
      Accept: "*/*",
...
function displayConversation(data) {
  ...
  combinedMessages.forEach((message) => {
    ...
    messageElement.textContent = `${
      message.type === "sent" ? "You" : studentName
